...
/* emulate 386 "pushal" instruction */
	...
	*pushsp = pptr->pesp = (unsigned long)saddr;
	...
	proctab[pid].pdbr = (FRAME0 + directoryFrame)*NBPG;
	frm_tab[directoryFrame].fr_type = FR_DIR;
	frm_tab[directoryFrame].fr_status = FRM_MAPPED;
	frm_tab[directoryFrame].fr_pid = pid;
	pd_t *Maindirectory = (pd_t *)((FRAME0+ directoryFrame)*NBPG);
	...
	for(i = 0; i < 4; i++)
	{
		...
		// point to existing golbal page table 
		Maindirectory[i].pd_base = FRAME0+i;
	}
	proctab[pid].vcreated = -1;
